...
class FieldExpectation extends SimpleExpectation
{
     ...
     *    @access private
     */
    public function _isSingle($value)
    {
     ...
     *    @access private
     */
    public function _testSingle($compare)
    {
     ...
     *    @access private
     */
    public function _testMultiple($compare)
    {
...
class HttpHeaderExpectation extends SimpleExpectation
{
     ...
     *    @access protected
     */
    public function _getExpectation()
    {
     ...
     *    @access private
     */
    public function _normaliseHeader($header)
    {
     ...
     *    @access protected
     */
    public function _findHeader($compare)
    {
        $lines = preg_split("/\r\n/D", $compare);
     ...
     *    @access private
     */
    public function _testHeaderLine($line)
    {
        if (count($parsed = preg_split('/:/D', $line, 2)) < 2) {
            return false;
        }
        [$header, $value] = $parsed;
     ...
     *    @access protected
     */
    public function _testHeaderValue($value, $expected)
    {
        if ($expected === false) {
            return true;
        }
...
class TextExpectation extends SimpleExpectation
{
    ...
    public function test($compare)
    {
        return (str_contains($compare, $this->_substring));
    }
     ...
     *    @access protected
     */
    public function _describeTextMatch($substring, $subject)
    {
...
class WebTestCase extends SimpleTestCase
{
     ...
     *    @access private
     */
    public function _failOnError($result)
    {
     ...
     *    @return boolean/string  Page on success.
     */
    public function submitFormById($id)
    {
     ...
     *    @access protected
     */
    public function _assertFieldValue($identifier, $value, $expected, $message)
    {
        if ($expected === true) {
            return $this->assertTrue(
                isset($value),
     ...
     *    @deprecated
     */
    public function assertHeaderPattern($header, $pattern, $message = '%s')
    {
        return $this->assert(
     ...
     *    @deprecated
     */
    public function assertNoUnwantedHeader($header, $message = '%s')
    {
     ...
     *	  @deprecated
     */
    public function assertNoUnwantedText($text, $message = '%s')
    {
     ...
     *	  @deprecated
     */
    public function assertNoUnwantedPattern($pattern, $message = '%s')
    {
